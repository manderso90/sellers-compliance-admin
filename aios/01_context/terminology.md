...
| **Requested Time**    | The customer’s desired time. Stored on `inspections.requested_time_preference`. As of 2026-04-26 the admin forms write an exact `HH:MM` string (e.g., `14:30`); the public order flow may still write legacy categorical values (`morning`, `afternoon`, `anytime`, `flexible`). Read-side display should use `getRequestedTimeLabel(value)` from `lib/utils/formatting.ts`, which formats `HH:MM` as 12-hour and capitalizes legacy strings. The DB CHECK (widened 2026-08-05) accepts the four legacy values or `HH:MM[:SS]`. |
