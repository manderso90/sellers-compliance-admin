...
**Severity**: Medium
**Update 2026-08-05**: CHECK-constraint audit found and fixed the worst piece of this drift — the DB allowed `hold` while all code (both repos' `JobStatus` type, `VALID_TRANSITIONS`, `JobStatusControl`) writes `on_hold`, so the On Hold button on the job detail page always failed with pg 23514. DB value renamed `hold` → `on_hold` (0 rows affected). The broader 11-vs-6 state drift below remains open. Same audit verified every other CHECK (customer_type, property_type, service_type, payment_status, inspector_outcome, profiles.roles, requested_time_preference) matches all write paths in both repos.
**Problem**: Live DB `inspections_status_check` allows 11 values (`requested`, `awaiting_confirmation`, `alternatives_offered`, `confirmed`, `in_progress`, `work_in_progress`, `completed`, `no_show`, `on_hold`, `needs_rescheduling`, `cancelled`). `aios/01_context/terminology.md`, `supabase/schema.sql` (pre-2026-04-22), and `services/job-lifecycle.ts#VALID_TRANSITIONS` all assume the narrower 6-state model (`requested` → `confirmed` → `in_progress` → `completed` | `cancelled` | `on_hold`). Hand-written UI and state-machine enforcement are out of step with the DB allow-list.
