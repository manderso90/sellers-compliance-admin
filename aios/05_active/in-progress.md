...
| Date | Item |
|------|------|
| 2026-08-05 | Work-only jobs + editable invoice (reported via the 401 Bernard St work-only job being invoiced the $125 inspection fee). Root cause: `getInspectionPrice` treated `price` 0/NULL identically — both fell back to the computed $125+ fee, so a fee could never be waived, and nothing considered `service_type`. Changes: (1) **pricing rule** — explicit `inspections.price` now always wins including 0; only NULL falls back to computed. Changed in BOTH repos' `lib/utils/pricing.ts` (public webhook uses it for `payment_status` math — keep in sync). (2) `createJob` sets `price = 0` when `service_type = 'work'`, so work-only jobs are born fee-waived (data-driven; no service-type conditionals downstream). (3) Job detail "Install Items" card is now **Invoice**: editable Inspection Fee row (new `updateInspectionFee` action → writes `inspections.price`; shows "Waived — work-only" at $0) + existing CRUD install items + Install Total + Invoice Total. (4) Stripe payment link product name says "Work Completion — {address}" for work jobs. (5) Data: Bernard job (`f16e4f08`) price set to 0 — no payment link had been created yet. No schema change (reused `price` column). |
